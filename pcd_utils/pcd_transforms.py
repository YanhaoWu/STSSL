...
def random_move_object(point_set, strength):
    """
    Randomly move the object to make it more robust
    ...
    for idx in range(0, len(different_label)):
        ...
        x_random = np.random.uniform(low=-strength, high=strength, size=1)
        y_random = np.random.uniform(low=-strength, high=strength, size=1)
        point_set[points_idx, 0] += x_random                     # 2022.7.7 做了修复
        point_set[points_idx, 1] += y_random
    return point_set                                              # 直接强行对聚类体进行平移，挪到不同的位置去
...
def random_sample_cluster(point_set, strength):
    ...
    """
    different_label = list(set(point_set[:, -1]))
    sample_points = np.zeros(shape=(1, 5))
    for idx in range(0, len(different_label)):
