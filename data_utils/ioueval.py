...
class iouEval:
  ...
  def addBatch(self, x, y):  # x=preds, y=targets
    # to tensor
    x_row = torch.from_numpy(x).to(self.conf_matrix.device).long()
    y_row = torch.from_numpy(y).to(self.conf_matrix.device).long()
    ...
    ones = torch.ones((idxs.shape[-1]), device=self.conf_matrix.device).long()
    # ones = torch.ones((idxs.shape[-1]), device=torch.device('cuda:0')).long()
